...
\begin{table}[h]
	...
	\begin{tabular}{cp{.05\linewidth}c}
		...
		\midrule
		I/O blocks connected to logic                                                                        & & \begin{tabular}[c]{@{}c@{}}input \emph{qubits} and output \\ \emph{qubits} that can be measured\end{tabular} \\
		\bottomrule
	\end{tabular}
...
\begin{itemize}
	...
	\item the symbol \mintinline{text}|$| is used to tag a \emph{qubit} as ancillary: unless explicitly requested by the programmer, the intermediate results are not reported in the solutions;
...
It is possible to verify, for each macro in Listings~\ref{lst:macros}, that given a configuration of the input \emph{qubits}, the value of \mintinline{text}|Y| that minimizes the energy corresponds to the output of the logic gate we are modeling. For\marginpar{Macro interpretation\\ as a polynomial} example, the explicit polynomial that describe \smallcaps{and} operator (where we have removed the $\$$ symbol for simplicity of reading) is:
\[
