...
\subsection{Reason}
As we have shown in Chapter~\ref{ch:qc}, programming a quantum computer is not an easy task. We express our algorithm in a very low-level way.
...
Even worse is the situation on quantum gate-based computers. The\marginpar{Quantum gates \\ and FPGAs} programmer has to build a quantum circuit gate by gate, an approach similar to what is done with FPGAs (Field Programmable Gate Arrays)~\cite{farooq2012fpga}. We can indeed see a strong analogy summed up in Table~\ref{tab:fpga}.

\begin{table}[h]
	\centering
	\begin{tabular}{cp{.05\linewidth}c}
		\toprule
		\textbf{FPGA}                                                                                       & & \textbf{Quantum gates computer}                                                                              \\
		\midrule
		\begin{tabular}[c]{@{}c@{}}programmable logic blocks\\  which implement logic functions\end{tabular} & & quantum gates                                                                                                \\
		\midrule
		\begin{tabular}[c]{@{}c@{}}programmable routing that \\ connects logic functions\end{tabular}        & & \begin{tabular}[c]{@{}c@{}}possibility to define \\ order of gates\end{tabular}                              \\
		\midrule
		I/O blocks connected to logic                                                                        & & \begin{tabular}[c]{@{}c@{}}input qubits and output \\ qubits that can be measured\end{tabular} \\
		\bottomrule
	\end{tabular}
	\caption{Parallelism between FPGA and quantum gate}
	\label{tab:fpga}
\end{table}
...
Today some \say{high-level gates} are available that wrap multiple low-level gates into useful patterns. There also exist, both for quantum gate-based computers and quantum annealers, some templates of well-known problems that need only fine-tuning to be useful for a specific problem. 

Despite these sorts of abstractions, programming a quantum computer is difficult and very close to machine language.

\subsection{Prolog}
We can see QA-Prolog as a compiler from Prolog to $\mathbf{H}_f$, where the ground state of $\mathbf{H}_f$ is the solution of our Prolog program. Before starting with the compilation process, it is useful to understand the main characteristics of Prolog, because it is not an imperative programming language like C or Java, but a \emph{declarative} one like SQL, moreover, Prolog\marginpar{Logic programming\\ language} is a \emph{logic programming language}. This means that the core of programming is not to tell the computer what to do, but to tell it what is true and ask it to try to draw conclusions~\cite{clocksin2003programming}. 

Discussing in detail the characteristics of Prolog and logic programming languages is out of scope of this work. More information about Prolog can be found in  \emph{Programming in PROLOG}~\cite{clocksin2003programming} and \emph{Learn Prolog Now!}~\cite{learn_prolog_now_online}. For the ones interested in logic programming language \emph{Logic for problem solving}\cite{kowalski1982logic} and \emph{Introduction to logic programming}\cite{hogger1984introduction} are recommended.
...
Programming\marginpar{Elements of a Prolog\\Program} in Prolog consists of:
\begin{itemize}
...
From these characteristics we can understand what \say{declarative} means: the program is a list of statements about our problem (our domain of interest); the relation between a Prolog program and an ontology is very strict. In Prolog we encode a KB\footnote{Knowledge Base} made of facts and rules; in Capter~\ref{ch:qa_ont} we can see a complete example of rewriting from an OWL ontology into a Prolog KB.

\paragraph{Arithmetic predicates:}\label{par:arithmetic} To better understand some QA-Prolog features that differ from basic Prolog we explain here how to assert equality between arguments when referring to integer. Predicates offered by Prolog are:
\begin{itemize}
	\item \mintinline{text}|+Expr1 =:= +Expr2|: true if expression \mintinline{text}|Expr1| evaluates to a number equal to \mintinline{text}|Expr2|;
	\item   \mintinline{text}|-Number is +Expr|: true when \mintinline{text}|Number| is the value to which \mintinline{text}|Expr| evaluates.
\end{itemize}
Where\marginpar{Arguments mode\\indicator} signs before arguments are \emph{arguments mode indicators}. Sign \mintinline{text}|+| specifies that the following argument must be instantiated, \mintinline{text}|-| indicates that the argument is a output~\cite{SWI2026PrologManual}. This mean that we cannot have variables when using \mintinline{text}|=:=| predicate because everything must be known at the time of evaluation. Using \mintinline{text}|is| the only variable can be the value of \mintinline{text}|Number|; \mintinline{text}|Expr|, on the other hand, has to be known.

\paragraph{Example:} Now we present a basic Prolog program in order to show the syntax and the usage of queries. We encode a KB about a party, we describe two people, Yolanda and Mia, asserting what they are doing, and giving some rules to derive other information about our small scenario.
\begin{listingenv}[h]
...
\subsection{Features of QA-Prolog}
...
QA-Prolog also supports some features not present in the basic version of Prolog. In particular, operations can be performed on variables even before they are ground~\cite{pakin2018performing}; this means that QA-Prolog is more powerful in manipulating free variables.  We can understand this feature looking back to Paragraph~\ref{par:arithmetic} where we have shown that aritmetica expression can have a free variable at most \textemdash{}\mintinline{text}|is| predicate\textemdash{} on the left operand. 

In\marginpar{Hints of reversible\\computation} QA-Prolog we can have variables on the left and on the right of predicate\footnote{In Prolog is possible to obtain the same result with library like CLP(FD)~\cite{triska2012finite, triska_clpfd_github} and CLP($\mathbb{Z}$)~\cite{triska_clpz_github}}, that way we can pass the input to the formula to obtain the result, but also assign a value to the result to retreive the input. We can already see that QA-Prolog allows a reversible computation: from input to output but also from output to input.
...
\begin{enumerate}
	...
	\item Low-level digital circuit in EDIF format;
	\item Symbolic Hamiltonian in qmasm formalism;
	\item Physical Hamiltonian.
	\item Execution un a \emph{quantum processing unit}
\end{enumerate}
\vspace{.2cm}
...
This software can be considered an assembler in the sense that it maps a symbolic representation of the operations (assembly language) to the machine language. QMASM\marginpar{QMASM as macro\\assembler} is not only an assembler, but extends its functionality by including macros: parameterized named blocks of assembly language that a program can instantiate multiple times~\cite{pakin2016quantum}.

\paragraph{Feature:} QMASM provsingides a number of features to simplify low-level D-Wave programming~\cite{pakin2016quantum}. Moreover, we can also use QMASM to assemble a physical Hamiltonian that we can later manipulate or solve using classical methods or other quantum systems.

Some of the most useful and interesting features of QMASM are:
\begin{itemize}
	\item qubits are referenced symbolically, not numerically, both in the source code and when QMASM reports execution results;
	\item qubits can be pinned to \mintinline{text}|true| or \mintinline{text}|false|;
	\item qubit patterns can be encapsulated into macros and instantiated repeatedly;
	\item groups of macros can be encapsulated into libraries and reused across multiple programs;
	\item QMASM can automatically exclude from the results the solutions known to be incorrect and shows only \say{interesting} qubits.
\end{itemize}

Thanks to thiings set of features, QMASM is already a useful abstraction layer that simplifies the development of programs that target an annealer, classical or quantum.

\paragraph{Example:}\marginpar{\\ Example of\\ satisfiability problem} Let us consider an example that shows the potential of QMASM and that is also useful for the following steps of the pipeline. The satisfiability problem is well known to be an NP-complete problem~\cite{garey2002computers}, so it is a good candidate for our example. We take into account the simple formula:
...
QMASM allows us to define a macro for every logical operator needed to represent the formula and then assemble the final formula by calling these macros. In Listings~\ref{lst:macros} we call \mintinline{text}|A| and \mintinline{text}|B| the input qubits and \mintinline{text}|Y| the output qubit. Weights are specified as an Insing problem (Section~\ref{sec:annealing_form}), which is the default format for QMASM source files\footnote{as specified in \url{https://github.com/lanl/qmasm/wiki/File-format}.}.

There are multiple interesting details about these macros:
\begin{itemize}
	\item the symbol \mintinline{text}|#| identifies a comment line not processed;
	\item the symbol \mintinline{text}|$| is used to tag a qubit as ancillary: unless explicitly requested by the programmer, the intermediate results are not reported in the solutions;
...
Compared with what we have done in Listing~\ref{lst:ising_ex}, defining weights in QMASM is much easier and the result is more readable;

It is possible to verify, for each macro in Listings~\ref{lst:macros}, that given a configuration of the input qubits, the value of \mintinline{text}|Y| that minimizes the energy corresponds to the output of the logic gate we are modeling. For\marginpar{Macro's interpretation\\ as a polynomial} example, the explicit polynomial that describe \smallcaps{and} operator (where we have removed the $\$$ symbol for simplicity of reading) is:
\[
	\mathbf{H} = -\frac{1}{2}\cdot(A + B) + Y + \frac{1}{2}\cdot(AB) - (AY + BY)
\]
Where $A, B, Y \in \set{-1, 1}$ because we are describing an Ising problem.

Listings~\ref{lst:macros}, also, show the usage of directive \mintinline{text}|!assert| (line $3$ of each listing): considering the \smallcaps{or} gate, we impose that in a valid solution $Y = A \lor B$, solutions that not verify this constraint are automatically discarded by QMASM.

We\marginpar{Assemble macros} can save these macros in a file named \mintinline{text}|gates.qmasm| and use it to solve our problem. To compute the formula $y = x_1 \land \neg(x_2 \lor x_3)$ we will use some intermediate results: we start with $x_4 = (x_2 \lor x_3)$, then we apply the negation $x_5 = \neg x_4$, and lastly we compute the result as $y = x_1 \land x_5$. 
...
The QMASM code implementing this procedure is reported in Listing~\ref{lst:circ_sat_qmasm}. This example shows how to use macros: we instantiate a macro and give it a name with \mintinline[escapeinside=@@]{text}|!use_macro @\textit{<macro\_name> <instance\_name>}@| (e.g. line $5$), and then instantiate the qubits defined in the macro with our actual qubits. For example, considering the \mintinline{text}|!use_macro OR| at line $5$, we can see that we use $x_2$ and $x_3$ as input and an ancillary qubit $\$x_4$ as output. Another detail to point out is the directive \mintinline{text}|!include| (line $3$), which imports all gates used in this source file.

Finally\marginpar{Run a QMASM\\\say{program}}, we can query the quantum annealer (or in this case a classical solver) to find a solution for our satisfiability problem. To do so we pin the output variable $y$ to be sure that in the solution its value will be \mintinline{text}|true|. We can query the solver with:
...
Verilog\marginpar{Verilog} is a hardware description language that offers different levels of design abstraction. The highest level is {\emph{behavioural} and uses programming constructs such as assignments, conditionals, and while-loops. The lowest level is a connection of gates (\emph{netlists})~\cite{gordon1995semantic}. 
...
\begin{itemize}
	\item they provide precise control over bit widths in order not to waste any qubit;
...
Figure~\ref{fig:yosys} shows a possible Verilog implementation of the formula~(\hyperref[lst:yosys_verilog]{a}), the netlist Yosys has produced from the Verilog code~(\hyperref[fig:yosys_net]{b}), and, only for clarification purposes, the corresponding digital circuit implemented with logic gates~(\hyperref[fig:yosys_circ]{c}).
...
Another detail to point out is the parameter \mintinline[escapeinside=@@]{text}|--work-dir="@\textit{<path>}@"|, which specifies the folder in which QA-Prolog outputs all files and the solver: we use a \emph{tabu search algorithm} because the simulated annealing implemented in Ocean is not capable of finding a solution even in this small scenario\footnote{More considerations about that in Chapter~\ref{ch:conclusion}.}. We say small because, from the study reported in \cite{zhang2010measuring}, we can observe that real-world ontologies define a number of classes, individuals, and properties\footnote{That we will transform in facts and rules in Chapter~\ref{ch:qa_ont}.} that can be on the order of hundreds of thousands.

By inspecting the working directory, we can check the output of each step of the pipeline. In Appendix~\ref{lst:qa_prolog_compilation} is shown the rewritten of a Prolog KB into a Verilog program: it is possible to observe in detail how each fact or rule has been transformed.
...
\paragraph{Example 2: }\label{ex:sat_prolog}\marginpar{\\ Satisfiability problem\\in QA-Prolog} The second example concludes our transformation of the satisfiability problem (Equation~\ref{eqn:simple_logic}); unfortunately, this is not very didactic because the Prolog program suitable for QA-Prolog processing introduces a consistent amount of overhead. This is due to the fact that in Prolog we can simply import \mintinline{text}|library(clpb)|~\cite{triska2018boolean} and then formulate the query \mintinline{text}|?- sat(A * (~(B + C))).|; to use QA-Prolog, instead, we have to implement logical operators by defining their truth tables, then assemble the operators in our logical formula. This leads to the overhead. Listing~\ref{lst:sat_prolog} shows the implementation we use in our test.
\begin{listingenv}[h]
...
\subsection{Overview}
We have described a pipeline of rewritings that, layer after layer, makes it possible to abstract away from the hardware that actually solves our problem, moving toward a language, and thus a way of reasoning, that is at a higher level. We\marginpar{Chain of rewritings} speak of rewritings because the object we are manipulating remains logically unchanged, just as it is essential that the solutions we are searching for remain unchanged; the problem and its solutions are simply expressed in different languages as one moves down the pipeline.
...
The\marginpar{Obsolete\\code} most fragile component of the pipeline is QMASM, because it is the one that interacts with external frameworks that are likely to change. In particular, the development of Ocean is very active, and some functions used in QMASM have now been removed, deprecated, or moved to other packages.
...
%		\mylistingnumbernear{.45}{Verilog}{code/qa_prolog/verilog_macro.vl}
%	}
%	\quad
%	\subfloat[Macro definitions]
%	{\label{lst:error_def}
%		\mylistingnumbernear{.45}{Verilog}{code/qa_prolog/qmasm_macro}
%	}\\
%	\subfloat[Macro usage]
%	{\label{lst:error_use}
%		\mylistingnumbernear{.7}{Verilog}{code/qa_prolog/qmasm_macro_use}
%	}
...
To\marginpar{Fixing macros' names} solve the problem we added a preprocessing step before parsing with QMASM. During the preprocessing all symbolic names are substituted with the actual name of the corresponding macro. 
