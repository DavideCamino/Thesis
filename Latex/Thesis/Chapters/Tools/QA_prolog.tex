...
Discussing in detail characteristics of Prolog and logic programming language are out of scope of this work, more information about Prolog can be found in  \emph{Programming in PROLOG}~\cite{clocksin2003programming} and \emph{Learn Prolog Now!}~\cite{learn_prolog_now_online}, for the ones interested in logic programming language \emph{Logic for problem solving}\cite{kowalski1982logic} and \emph{Introduction to logic programming}\cite{hogger1984introduction} are recommended.
...
From these characteristics we can understand what \say{declarative} means: the program is a list of statements about our problem (our domain of interest); the relation between a Prolog program and an ontology is very strict. In Prolog we encode a KB made of facts and rules; in Capter~\ref{} we can see a complete example of rewriting from an OWL ontology into a Prolog KB\footnote{Knowledge Base}.
...
\paragraph{Example:} Now we present a basic Prolog program in order to show the syntax and the usage of queries. We encode a KB about a party, we describe two people, Yolanda and Mia, asserting what are they doing and giving some rules to derive other informations about our small scenario.
\begin{listing}[h]
