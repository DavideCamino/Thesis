...
In conclusion, even if there exist some sort of abstractions with \say{high-level gates} that wrap multiple low-level gates into useful patterns and there exist, both for quantum gate-based computers and quantum annealers, some templates of well-known problems that need only fine-tuning to be useful for a specific problem, programming a quantum computer is, today, very close to machine language.

The goal of QA-Prolog is to fill the gap between the high-level description provided by a powerful logic programming language and promising quantum computers.

\subsection{Prolog}
We can see QA-Prolog as a compiler from Prolog to $\mathbf{H}_f$, where the ground state of $\mathbf{H}_f$ is the solution of our Prolog program. Before starting with the compilation process, it is useful to understand the main characteristics of Prolog, because it is not an imperative programming language like C or Java, but a declarative one. For more information about Prolog, the lectures of \cite{clocksin2003programming} and \cite{learn_prolog_now_online} are recommended.

In Prolog we do not specify step by step an algorithm that resolves our problem; instead we describe the formal relationship between the objects in our problem and which relations have to be true in our solution\cite{clocksin2003programming}.
...
For example, the rule at line seven can be read as: \say{If Mia is singing, then Mia is happy}. Line nine shows the usage of a variable; variables start with an uppercase letter and are placeholders for information. We can read this rule as \say{If someone is happy, he smiles}.
...
From a high-level point of view, the pipeline rewrites the initial knowledge base expressed in Prolog into different objects. The logical meaning of the entities we build during the pipeline is:
\begin{enumerate}
...
\begin{itemize}
	...
	\item QMASM can automatically exclude from the results the solutions known to be incorrect;
...
\paragraph{Example:} Let us consider an example that shows the potential of QMASM and that is also useful for the following steps of the pipeline. The satisfiability problem is well known to be an NP-complete problem\cite{bibid}, so it is a good candidate for our example. We take into account the simple formula:
...
QMASM allows us to define a macro for every logical operator needed to represent the formula and then assemble the final formula by calling these macros. In Listings \ref{lst:macro} we call \mintinline{text}|A| and \mintinline{text}|B| the input \emph{qubits} and \mintinline{text}|Y| the output \emph{qubit}. Weights are specified as an Ising problem \ref{label}, which is the default format for QMASM source files\footnote{as specified in \url{https://github.com/lanl/qmasm/wiki/File-format}}.
...
\begin{itemize}
	...
	\item the sign \mintinline{text}|$| is used to tag a \emph{qubit} as ancillary; this means that, unless explicitly requested by the programmer, the intermediate results are not reported in the solutions;
...
\end{itemize}
It is possible to verify, for each macro in listings \ref{lst:macro}, that given a configuration of the input \emph{qubits}, the value of \mintinline{text}|Y| that minimizes the energy corresponds to the output of the logic gate we are modeling.
...
This example shows how to use macros: we instantiate a macro and give it a name with \mintinline[escapeinside=@@]{text}|!use_macro @\textit{<macro\_name> <instance\_name>}@| (e.g., line five), and then instantiate the \emph{qubits} defined in the macro with our actual \emph{qubits}. For example, considering the \mintinline{text}|!use_macro OR| at line five, we can see that we use $x_2$ and $x_3$ as input and an ancillary \emph{qubit} $\$x_4$ as output. Another detail to point out is the directive \mintinline{text}|!include| (line three), which imports all gates used in this source file.
...
Verilog is a hardware description language that offers different levels of design abstraction. The highest level is \say{behavioural} and uses programming constructs such as assignments, conditionals, and while-loops. The lowest level is a connection of gates (netlists)~\cite{gordon1995semantic}. 
...
In these steps of the pipeline we follow the same procedure used in the design of digital systems: we start using behavioural constructs, then we automatically generate a netlist as the output of synthesisers. 

The synthesiser used is Yosys~\cite{wolf2013yosys}, the first free and open-source software for Verilog HDL synthesis, which supports the vast majority of synthesizable Verilog features.

The two most useful features of Yosys are the possibility of specifying a \emph{cell library}, the set of gates used to synthesize the netlist, and the use of the external tool Berkeley ABC~\cite{brayton2010abc} (incorporated in Yosys), providing additional code optimizations.
...
\paragraph{Example:} Considering again the simple formula $y = x_1 \land \neg(x_2\lor x_3)$ (Equation \ref{eqn:simple_logic}), in Figure \ref{fig:yosys} we can see in (\hyperref[fig:yosys_verilog]{a}) a possible Verilog implementation of the formula, in (\hyperref[fig:yosys_net]{b}) the netlist Yosys has produced from the Verilog code, and in (\hyperref[fig:yosys_circ]{c}), only for clarification purposes, the corresponding digital circuit implemented with logic gates.
...
\begin{figure}[h]
	...
	{\label{fig:yosys_verilog}
		\begin{minipage}{.45\linewidth}
			\vspace{1.5cm}
			\inputminted{Verilog}{code/qa_prolog/sat.vl}
		\end{minipage}
	} \qquad
...
Now we can feed the resulting netlist to edif2qmasm to obtain:
\begin{listing}[h]
	\begin{minted}{text}
		!include <stdcell>
		
		!begin_macro sat
		!use_macro AND $id00006  # $auto$simplemap.cc:83:simplemap_bitop$83
		!use_macro NOT $id00004  # $auto$simplemap.cc:38:simplemap_not$82
		!use_macro OR $id00005  # $auto$simplemap.cc:83:simplemap_bitop$81
		$id00004.A = $id00005.Y  # $or$sat.v:5$1_Y
		$id00005.A = b
		$id00005.B = c
		$id00006.A = a
		$id00006.B = $id00004.Y  # $not$sat.v:5$2_Y
		$id00006.Y = y
		!end_macro sat
...
\begin{itemize}
	\item in listing \ref{lst:circ_sat_yosys} we define the new macro \mintinline{text}|sat|; this is because each module in the Prolog program is converted into a macro;
...
\paragraph{Examples: } The first example we present shows a small knowledge base that can be summarized as \say{the enemy of my enemy is my friend}. 
...
We can now query the KB to find if there are two people who are friends; we can do that with:
\begin{minted}[linenos=false]{text}
	QA-Prolog --qmasm-args="--solver=tabu" --query='friends(P1, P2).' --work-dir="~/work" friends.pl
\end{minted}

In this command we can see that QA-Prolog allows us to query the KB with the exact same syntax we would use to interact with a Prolog interpreter. As expected, QA-Prolog binds the variables \mintinline{text}|P1| and \mintinline{text}|P2| to Alice and Charlie.

Another detail to point out is the parameter \mintinline[escapeinside=@@]{text}|--work-dir="@\textit{<path>}@"|, which specifies the folder in which QA-Prolog outputs all files, and the solver: we use a tabu search algorithm because the simulated annealing implemented in Ocean is not capable of finding a solution even in this small scenario (more considerations about that in the concluding Chapter \ref{}).

If we inspect the working directory we can see the output of every step of the pipeline; in particular, to see how exactly the KB is rewritten into a Verilog program, in Appendix \ref{} there is the result of compilation, and it is possible to observe in detail how each fact or rule has been transformed.
...
We can now run QA-Prolog with the query \mintinline{text}|sat(A, B, C, true)| to find the correct assignment of Boolean variables that satisfy our logic formula. Again, we are forced to use the tabu search algorithm.
...
\begin{itemize}
	\item in the \mintinline{text}|dwave.cloud| and \mintinline{text}|hibrid| libraries;
...
\begin{figure}[h]
	...
	{\label{lst:error_verilog}
		\begin{minipage}{.45\linewidth}
			\inputminted{Verilog}{code/qa_prolog/verilog_macro.vl}
		\end{minipage}
	}
	\qquad
	...
	{\label{lst:error_def}
		\begin{minipage}{.45\linewidth}
			\inputminted{Verilog}{code/qa_prolog/qmasm_macro}
		\end{minipage}
	}\\
	\subfloat[Macro usage]
	{\label{lst:error_use}
		\begin{minipage}{.7\linewidth}
			\inputminted{Verilog}{code/qa_prolog/qmasm_macro_use}
		\end{minipage}
	}
	\captionof{listing}{text}
	\label{lst:qmasm_error}
\end{figure}

The error can be seen in Listings \ref{lst:qmasm_error}: edif2qmasm rewrites the netlist generated from (\hyperref[lst:error_verilog]{a}) into the macro (\hyperref[lst:error_def]{b}); here \mintinline{text}|hates/2| is just a comment, the actual name of the macro is \mintinline{text}|id00011|. In (\hyperref[lst:error_verilog]{c}), however, we can see that the macro previously defined is called symbolically and not with its actual name (lines $4$ and $5$). This obviously produces an error: the name \mintinline{text}|hates/2| in the QMASM file has no meaning, it is only a comment.

\begin{listing}[h]
	\inputminted{py}{code/qa_prolog/preprocess.py}
	\caption{text}
...
In order to have a working environment where we can run our experiments we need to install all the software required by the QA-Prolog pipeline. In Chapter \ref{ch:env} we have set up a python environment with all the essential tools; to start the installation of the QA-Prolog pipeline we need at least the Ocean SDK installed as described in \ref{sec:ocean}.
...
Next we need Yosys and GO; Yosys is part of the pipeline, and GO allows us to compile edif2qmasm and QA-Prolog. Both Yosys and GO are available as binary packages for the majority of GNU/Linux distributions; on Arch\footnote{https://archlinux.org/} they can be installed with:
\begin{minted}{text}
	# pacman -S yosys
	# pacman -S go
\end{minted}
