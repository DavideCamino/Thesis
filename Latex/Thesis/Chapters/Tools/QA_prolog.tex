...
\paragraph{Arithmetic predicates:} To better understand some QA-Prolog features that differ from basic Prolog we explain here how to assert equality between arguments when referring to integer. Predicates offered by Prolog are:
\begin{itemize}
...
This feature is useful when manipulating aritmetic operation, in basic prolog we have two operator to express the equality between two expression that are \mintinline{text}|+Expr1 =:= +Expr2| and \mintinline{text}|-Number is +Expr|, where the sign before arguments is the \emph{argument mode indicator}:
\begin{itemize}
	\item \mintinline{text}|+| is limited because both expression must be known at the time of evaluation;
	\item \mintinline{text}|-| is limited because both expression must be known at the time of evaluation;
\end{itemize}
